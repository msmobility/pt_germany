 ...
 * @author mrieser / SBB
 *
 * And carlos llorca has added the distance on toll highways for an specific application
 */
public class LeastCostPathTreeWithTolls {
    ...
    public LeastCostPathTreeWithTolls(TravelTime tt, TravelDisutility tc) {
        this.ttFunction = tt;
        this.tcFunction = tc;
    }
    ...
    public static class NodeData {
        ...
        private double distanceWithTolls = 0;

        /*package*/ void visit(final Id<Node> comingFromNodeId, final double cost1, final double time1, double distance, double distanceWithTolls) {
            this.prevId = comingFromNodeId;
            this.cost = cost1;
            this.time = time1;
            this.distance = distance;
            this.distanceWithTolls = distanceWithTolls;
        }
        ...
        public double getDistanceWithTolls() {
            return distanceWithTolls;
        }
    }
       ...
       NodeData nData = nodeData.get(n.getId());
        ...
        for (Link l : n.getOutLinks().values()) {
            ...
            if (visitCost < nnData.getCost()) {
                ...
                nnData.visit(n.getId(), visitCost, visitTime, distance, distanceWithToll);
    ...
    public static void main(String[] args) {
        ...
        ch.sbb.matsim.analysis.skims.LeastCostPathTree st = new ch.sbb.matsim.analysis.skims.LeastCostPathTree(ttc.getLinkTravelTimes(), new RandomizingTimeDistanceTravelDisutilityFactory( TransportMode.car, scenario.getConfig().planCalcScore() ).createTravelDisutility(ttc.getLinkTravelTimes()));
        Node origin = network.getNodes().get(Id.create(1, Node.class));
        st.calculate(network, origin, 8*3600);
        Map<Id<Node>, ch.sbb.matsim.analysis.skims.LeastCostPathTree.NodeData> tree = st.getTree();
        for (Map.Entry<Id<Node>, ch.sbb.matsim.analysis.skims.LeastCostPathTree.NodeData> e : tree.entrySet()) {
            Id<Node> id = e.getKey();
            ch.sbb.matsim.analysis.skims.LeastCostPathTree.NodeData d = e.getValue();
